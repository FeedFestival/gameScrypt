...
import { COOKIES_TERMS } from '../privacy-policy-docs/cookies-docs';
...
export class CookiesComponent implements OnInit, OnChanges {

    @Input() languageLogic = true;
    @Input() lang: string;

    langDecided = false;
    PRIVACY_POLICY_COOKIES: COOKIES_TERMS;
    ...
    ngOnChanges(changes: SimpleChanges) {
        if (changes) {
            if (_isNil(changes.lang) === false && _isNil(changes.lang.currentValue) === false) {
                this.PRIVACY_POLICY_COOKIES = new PrivacyPolicy(this.sanitized)._().COOKIES[this.lang];
            }
        }
    }
    ...
    private changeLang(lang, navigate: boolean = true) {
        ...
        if (navigate) {
            this.appEventManager.broadcast(new EventContent(EVENT.NAVIGATE, {
                goRoute: TERMS_PRIVACY.COOKIES + (lang === COUNTRIES.OTHER.lang ? '' : '/ro')
            }));
        } else {
            this.lang = lang;
            this.PRIVACY_POLICY_COOKIES = new PrivacyPolicy(this.sanitized)._().COOKIES[lang];
        }
    }
}
