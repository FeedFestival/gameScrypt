...
import { GDPR_TERMS } from '../privacy-policy-docs/gdpr-docs';
...
export class GdprComponent implements OnInit, OnChanges {

    @Input() languageLogic = true;
    @Input() lang: string;
    langDecided = false;

    PRIVACY_POLICY_GDPR: GDPR_TERMS;
    ...
    ngOnChanges(changes: SimpleChanges) {
        if (changes) {
            if (_isNil(changes.lang) === false && _isNil(changes.lang.currentValue) === false) {
                this.PRIVACY_POLICY_GDPR = new PrivacyPolicy(this.sanitized)._().GDPR[this.lang];
            }
        }
    }
    ...
    private changeLang(lang, navigate: boolean = true) {
        ...
        if (navigate) {
            this.appEventManager.broadcast(new EventContent(EVENT.NAVIGATE, {
                goRoute: TERMS_PRIVACY.GDPR + (lang === COUNTRIES.OTHER.lang ? '' : '/ro')
            }));
        } else {
            this.lang = lang;
            this.PRIVACY_POLICY_GDPR = new PrivacyPolicy(this.sanitized)._().GDPR[lang];
        }
    }
}
